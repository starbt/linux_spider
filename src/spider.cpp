...
int main(int argc, void *argv[]) 
{
    ...
    int n, i;
    while(1) {
        n = epoll_wait(g_epfd, events, 20, 1000);
		printf("epoll wait....\n");
		fflush(stdout);
        if (n == -1) {
        ...
        for (i = 0; i < n; i++) {
            ...
            printf("the arg is :domain:%s path:%s ip:%s\n",arg->url->domain,arg->url->path,arg->url->ip);
			print_stat();
            fflush(stdout);
...
static void daemonize()
{
    ...
    if (g_conf->logfile != NULL && (fd = open(g_conf->logfile, O_RDWR | O_APPEND | O_CREAT, 0777)) != -1) {
        dup2(fd, STDOUT_FILENO);
        if (fd > STDERR_FILENO)
            close(fd);
    }
