...
pthread_cond_t  sq_notfull_cond = PTHREAD_COND_INITIALIZER;
pthread_cond_t  sq_notempty_cond = PTHREAD_COND_INITIALIZER;

pthread_cond_t oq_notfull_cond = PTHREAD_COND_INITIALIZER;
pthread_cond_t oq_notempty_cond = PTHREAD_COND_INITIALIZER;

void push_surlqueue(Surl *url)
{
    if (url != NULL && surl_precheck(url)) {
        SPIDER_LOG(SPIDER_LEVEL_DEBUG, "I find a url: %s", url->url);
        pthread_mutex_lock(&sq_lock);
	while (surl_queue.size() == SQUEUE_MAX_SIZE) {
		SPIDER_LOG(SPIDER_LEVEL_WARN,"the surl_queue is full.\n");
		pthread_cond_wait(&sq_notfull_cond, &sq_lock);
	}
        surl_queue.push(url);
        if (surl_queue.size() == 1)
            pthread_cond_broadcast(&sq_notempty_cond);
...
Surl * pop_surlqueue()
{
    Surl *surl = NULL;
	pthread_mutex_lock(&sq_lock);
	while (surl_queue.size()==0) {
		SPIDER_LOG(SPIDER_LEVEL_WARN,"the surl_queue is empty.\n");
		pthread_cond_wait(&sq_notempty_cond, &sq_lock);
	}
	surl = surl_queue.front();
	surl_queue.pop();
	pthread_cond_signal(&sq_notfull_cond);
	pthread_mutex_unlock(&sq_lock);
	return surl;
}
...
static void push_ourlqueue(Url * ourl)
{
	...
	while (ourl_queue.size() == OQUEUE_MAX_SIZE) {
		SPIDER_LOG(SPIDER_LEVEL_WARN,"the ourl_queue is full.\n");
		pthread_cond_wait(&oq_notfull_cond, &oq_lock);
	}
    ourl_queue.push(ourl);
    if (ourl_queue.size() == 1)
        pthread_cond_broadcast(&oq_notempty_cond);
...
Url * pop_ourlqueue()
{
    ...
    if (!ourl_queue.empty()) {
        ...
        while (trynum-- && ourl_queue.empty()) {
            ...
            pthread_cond_timedwait(&oq_notempty_cond, &oq_lock, &timeout);
        }
		...
		pthread_cond_signal(&oq_notfull_cond);
        pthread_mutex_unlock(&oq_lock);
        return url;
    }
}
...
void * urlparser(void *none)
{
    ...
    while(1) {
        ...
        if (itr == host_ip_map.end()) { /* not found */
			...
			ip_address = gethostbyname(ourl->domain);
			if (ip_address == NULL){
            	printf("DNS解析错误\n");
			}else {
				char ipstr[16];
				inet_ntop(AF_INET, (ip_address->h_addr_list)[0], ipstr, 16);
				printf("the ip is %s.\n",ipstr);
				ourl->ip = strdup(ipstr);
				push_ourlqueue(ourl);
			}
        } else {
